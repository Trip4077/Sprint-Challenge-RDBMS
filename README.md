...
A RDBMS is a mangement system used to control and modify data within a database, while SQL is 
language used to intereact with that system and database

1. Why do tables need a `primary key`?

A primary key is needed to provide a unique value to each row in a table that can be
used to identify rows by value.
...
A bridge or joining table that holds the two tables' primary keys as foreign keys.
